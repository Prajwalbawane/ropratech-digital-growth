  ...
  <>
      ...
      <div className="container max-w-3xl text-center relative">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-gold/10 border border-gold/20 text-sm text-gold mb-6">
            <Zap className="h-4 w-4 text-gold" /> What We Offer
          </div>
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-black">Our Services</h1>
          <p className="mt-6 text-white/50 text-lg">Simple digital solutions designed for local businesses. No jargon, no confusion.</p>
        </motion.div>
      </div>
    </section>
          ...
          <FadeIn key={s.title}>
            <div className={`grid gap-8 lg:grid-cols-5 items-center`}>
              <div className={`lg:col-span-3 ${i % 2 === 1 ? "lg:order-2" : ""}`}>
                ...
                <h2 className="text-2xl md:text-3xl font-heading font-black">{s.title}</h2>
                <p className="text-muted-foreground mt-3 leading-relaxed text-lg">{s.desc}</p>
                <p className="mt-4 font-bold text-xl text-gold">Starting from {s.price}</p>
              </div>
              <div className={`lg:col-span-2 royal-card ${i % 2 === 1 ? "lg:order-1" : ""}`}>
                ...
                <ul className="space-y-3">
                  {s.useCases.map((u) => (
                    <li key={u} className="flex items-start gap-3 text-sm">
                      <CheckCircle className="h-4 w-4 shrink-0 mt-0.5 text-gold" />
                      <span>{u}</span>
                    </li>
                  ))}
                </ul>
    ...
    <section className="py-16 hero-gradient text-white relative overflow-hidden">
      <div className="container text-center relative">
        <FadeIn>
          <h2 className="text-3xl md:text-4xl font-heading font-black">Ready to get started?</h2>
          <p className="mt-3 text-white/50 text-lg">Let's discuss your project — no commitment, no pressure.</p>
          <div className="mt-8 flex flex-wrap justify-center gap-4">
            <a href="https://wa.me/919999999999?text=Hi%20RoPratech" target="_blank" rel="noopener noreferrer">
              <Button size="lg" className="bg-whatsapp hover:bg-whatsapp/90 text-white gap-2 rounded-xl shadow-xl shadow-black/20 h-12 px-6">
                <MessageCircle className="h-5 w-5" /> WhatsApp Us
              </Button>
            </a>
            <Link to="/contact">
              <Button size="lg" className="btn-royal gap-2 rounded-xl h-12 px-6">
